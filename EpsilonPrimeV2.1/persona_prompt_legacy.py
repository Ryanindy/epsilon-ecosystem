...
# NEW: Import RAG retrieval logic
...
def generate_ai_response(sender_number: str, user_message: str, context: Union[List[str], Dict]) -> str:
    ...
    # 1. Fetch RAG Context (NEW UPGRADE)
    ...
    for model_name in MODELS_TO_TRY:
        try:
            ...
            # If it's a quota error or overloaded, try next model
            if "429" in error_str or "quota" in error_str.lower() or "overloaded" in error_str.lower():
                # print(f"[DEBUG]: {model_name} failed (Quota/Overload). Cascading...")
                continue
            else:
                # If it's a different error, we might still want to try other models
                # unless it's a bad request/unauthorized
